...
describe("Ballot", function () {
  ...
  describe("when the voter interact with the vote function in the contract", function () {
    // TODO
    it("should revert vote if vouter weight is 0", async function () {
      // const voterAdress = accounts[1].address;
      const voterAddress = accounts[1].address;
      await giveRightToVote(ballotContract, voterAddress);
      const proposal = (await ballotContract.proposals(0)).name;
      await vote(ballotContract, proposal)
      // await expect( vote(ballotContract, proposal)).to.be.revertedWith("Has no right to vote");
    });
  });
